...
## 28. CAJO Inventory — currency selector on Purchase Orders

**Status: backend built, applied to the live database.**

```
inv_purchases now has a `currency` field (ISO 4217 code, e.g. "USD", "INR", "EUR").

GET  /api/:slug/inventory/purchases  |  GET /purchases/:id  |  GET /purchases/all-items
  Every purchase object now includes: currency (string, e.g. "INR")

POST /api/:slug/inventory/purchases — body gains: currency? (optional)
  Defaults to the selected vendor's own currency (inv_vendors.currency) if omitted — so the
  dropdown can pre-fill from the vendor once one's picked, but the user can still change it for
  this specific PO.

PATCH /api/:slug/inventory/purchases/:id — body gains: currency?
```

Add a "Currency" dropdown to both the Make Purchase and Edit Purchase forms (header section,
alongside Vendor/PO Number/Purchase Date), pre-filled from the selected vendor's currency but
changeable. Use the full ISO 4217 list below — paste directly:
...
const CURRENCIES = [
  ...
  { code: 'MMK', name: 'Myanma Kyat' }, { code: 'MNT', name: 'Mongolian Tugrik' },
  ...
  { code: 'PLN', name: 'Polish Złoty' }, { code: 'PYG', name: 'Paraguayan Guarani' },
  ...
  { code: 'STN', name: 'São Tomé and Príncipe Dobra' }, { code: 'SYP', name: 'Syrian Pound' },
  { code: 'SZL', name: 'Swazi Lilangeni' }, { code: 'THB', name: 'Thai Baht' },
  { code: 'TJS', name: 'Tajikistani Somoni' }, { code: 'TMT', name: 'Turkmenistani Manat' },
  { code: 'TND', name: 'Tunisian Dinar' }, { code: 'TOP', name: "Tongan Pa'anga" },
